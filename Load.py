...
DATAPATH = osp.dirname(osp.abspath(__file__)) + '/dataset/'

def load(dataset):
    
    data = np.load(dataset + '_directed.npz')
    """
    # Get the names of arrays stored in the .npz file
    array_names = data.files

    # Print the names of arrays
    print("Arrays stored in the .npz file:", array_names)
    """
    node_features = torch.tensor(data['node_features'], dtype=torch.float32)
    edges = torch.tensor(data['edges'], dtype=torch.long) # Assuming edges are stored as an (N, 2) array
    labels = torch.tensor(data['node_labels'], dtype=torch.long)
    graph = Data(x=node_features, edge_index=edges.t().contiguous(), y=labels)

    num_classes = torch.unique(torch.tensor(data['node_labels'])).numel()
    
    if 'train_masks' in data:
        train_masks = torch.tensor(data['train_masks'], dtype=torch.bool).t().contiguous()
        graph.train_mask = train_masks

    if 'val_masks' in data:
        val_masks = torch.tensor(data['val_masks'], dtype=torch.bool).t().contiguous()
        graph.val_mask = val_masks

    if 'test_masks' in data:
        test_masks = torch.tensor(data['test_masks'], dtype=torch.bool).t().contiguous()
        graph.test_mask = test_masks
...
def load_fb100(filename):
    ...
    mat = scipy.io.loadmat(DATAPATH  + 'penn/' + 'Penn94' + '.mat')
    A = mat['A']
    metadata = mat['local_info']
    return A, metadata

def load_pokec():
    pathname = f"{DATAPATH}pokec/Pokec"
